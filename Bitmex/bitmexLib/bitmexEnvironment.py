...
class bitmexEnvironment(gym.Env):
    ...
    def step(self, action_id):
        ...
        price = self.df.iloc[self.current_index]['C']
        price_next = self.df.iloc[next_index]['C']
        ...
        self.gain = (asset - self.initial_account) / \
            self.initial_account * 100
        ...
        next_state = np.append(next_state_line.to_numpy(
        ), self.pos, axis=None).reshape(1, self.state_size)
    ...
    def render(self, mode='human', close=False):
        if self.current_index % 100 == 0:
            if self.episode_old != self.episode:
                ...
                print('--------------v0----------------')
            print("episode: {}, time: {}, gain: {:.2f}, trades: {}, buys : {}, sells : {}"
                  .format(self.episode, self.current_index, self.gain, self.trades, self.buyActions, self.sellActions))
    ...
    def __get_state(self):
        ...
        state = np.append(state_line.to_numpy(), self.pos,
                          axis=None).reshape(1, self.state_size)
    ...
    def showGraph(self):
        plt.plot(self.account, label="Gain")
        plt.show()
